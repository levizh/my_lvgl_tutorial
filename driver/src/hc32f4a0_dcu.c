 ...
 * @brief This file provides firmware functions to manage the DCU.
 @verbatim
   Change Logs:
   Date             Author          Notes
   2020-02-10       Hongjh          First version
 @endverbatim
 *******************************************************************************
 ...
 *         This parameter can be one of the following values:
 *           @arg  DCU_COM1_TRIG_DISABLE: Disable common trigger event 1 for the specified usage.
 *           @arg  DCU_COM1_TRIG_ENABLE:  Enable common trigger event 1 for the specified usage.
 *           @arg  DCU_COM2_TRIG_DISABLE: Disable common trigger event 2 for the specified usage.
 *           @arg  DCU_COM2_TRIG_ENABLE:  Enable common trigger event 2 for the specified usage.
 * @retval None
 */
en_result_t DCU_ComTrigCmd(M4_DCU_TypeDef *DCUx, uint32_t u32ComTrigEn)
{
    ...
    if (NULL != TRGSELx)
    {
        MODIFY_REG32(*TRGSELx, \
                     (AOS_DCU_1_TRGSEL_COMTRG_EN_0 | AOS_DCU_1_TRGSEL_COMTRG_EN_1), \
                     u32ComTrigEn);
        enRet = Ok;
    }
...
uint8_t DCU_ReadReg8Data0(M4_DCU_TypeDef *DCUx)
{
    ...
    return READ_REG8(*(__IO uint8_t *)(&DCUx->DATA0));
}
...
uint8_t DCU_ReadReg8Data1(M4_DCU_TypeDef *DCUx)
{
    ...
    return READ_REG8(*(__IO uint8_t *)(&DCUx->DATA1));
}
...
uint8_t DCU_ReadReg8Data2(M4_DCU_TypeDef *DCUx)
{
    ...
    return READ_REG8(*(__IO uint8_t *)(&DCUx->DATA2));
}
...
uint16_t DCU_ReadReg16Data0(M4_DCU_TypeDef *DCUx)
{
    ...
    return READ_REG16(*(__IO uint16_t *)(&DCUx->DATA0));
}
...
uint16_t DCU_ReadReg16Data1(M4_DCU_TypeDef *DCUx)
{
    ...
    return READ_REG16(*(__IO uint16_t *)(&DCUx->DATA1));
}
...
uint16_t DCU_ReadReg16Data2(M4_DCU_TypeDef *DCUx)
{
    ...
    return READ_REG16(*(__IO uint16_t *)(&DCUx->DATA2));
}
