...
#define SDIOC_ARG(__SDIOCx__)                   ((uint32_t)(&((__SDIOCx__)->ARG0)))
#define SDIOC_BUF(__SDIOCx__)                   ((uint32_t)(&((__SDIOCx__)->BUF0)))
...
static en_result_t SDMMC_WaitResponse(M4_SDIOC_TypeDef *SDIOCx, uint32_t u32CheckFlag, uint32_t u32Timeout, uint32_t *pu32ErrSta)
{
    ...
    *pu32ErrSta = 0UL;
    ...
    {
        enRet = Error;
        if (Reset == SDIOC_GetIntStatus(SDIOCx, CLEAR_REG32_BIT(u32CheckFlag, SDIOC_NORMAL_INT_FLAG_CC)))
        {
            /* No error flag set */
            *pu32ErrSta = SDMMC_ERROR_NONE;
...
static en_result_t SDMMC_GetCmdResp1(M4_SDIOC_TypeDef *SDIOCx, uint32_t u32Timeout, uint32_t *pu32ErrSta)
{
    ...
    if (Ok == enRet)
    {
        ...
        SDIOC_GetResponse(SDIOCx, SDIOC_RESPONSE_REG_BIT0_31, &u32RespVal);
