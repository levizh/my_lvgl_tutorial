...
#if (APP_AWD_USE_COMB_MODE > 0U)
    #define APP_ADC_AWD_COMB_IRQ_CB         ADC_2_Cmp1Comb_IrqHandler
...
#else
    #define APP_ADC_AWD0_IRQ_CB             ADC_2_Cmp0_IrqHandler
    #define APP_ADC_AWD1_IRQ_CB             ADC_2_Cmp1Ind_IrqHandler
...
static void AdcTrigSrcConfig(void)
{
    ...
    stcInit.u32ClkDiv = TMR2_CLK_DIV_128;
...
static void AdcIrqConfig(void)
{
    ...
    stcCfg.pfnCallback = APP_ADC_AWD_COMB_IRQ_CB;
    ...
    stcCfg.pfnCallback = APP_ADC_AWD0_IRQ_CB;
    if (stcCfg.enIRQn == ADC_SHARE_IRQn)
    {
        /* Sharing interrupt. */
        INTC_ShareIrqCmd(stcCfg.enIntSrc, Enable);
    ...
    stcCfg.pfnCallback = APP_ADC_AWD1_IRQ_CB;
    if (stcCfg.enIRQn == ADC_SHARE_IRQn)
    {
        /* Sharing interrupt. */
        INTC_ShareIrqCmd(stcCfg.enIntSrc, Enable);
 ...
 * @retval None
 */
void APP_ADC_AWD_COMB_IRQ_CB(void)
{
