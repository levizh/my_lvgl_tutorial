...
static uint8_t CaptureCnt = 0U;
static uint8_t CAPTURE_CNT_MAX = 2U;
/*******************************************************************************
 ...
 * @retval None
 */
static void TMR0_2_ChBCapt_IrqCallback(void)
{       
    if(CaptureCnt != CAPTURE_CNT_MAX)
    {
        CaptureData[CaptureCnt] = TMR0_GetCmpVal(TMR0x, TMR0_CH);
        CaptureCnt++;
    }
    else
    {
        /* Finish timer0 capture */
        TMR0_Cmd(TMR0x, TMR0_CH, Disable);
        /* Timer0 interrupt function Disable */
        TMR0_IntCmd(TMR0x, TMR0_CH, Disable);
    }
...
static void TMR0_Config(void)
{
    ...
    stcTmr0Init.u32Tmr0Func = TMR0_FUNC_CAPTURE;        /* Timer0 compare mode */
...
static void EXINT_IRQ_Config(void)
{
    ...
    stcIrqSignConfig.pfnCallback = &TMR0_2_ChBCapt_IrqCallback;
