...
#define TMR4_CNT_CYCLE_VAL(div)         ((uint16_t)(Tmr4PclkFreq() /  (2U * (1UL << (uint32_t)(div))))) /* Period_Value(500ms) */

/* Timer4 OCO Channel definition */
#define TMR4_OCO_CH                   (TMR4_OCO_UL)    /* only TMR4_OCO_UL  TMR4_OCO_VL  TMR4_OCO_WL */
...
int32_t main(void)
{
    ...
    if (TMR4_OCO_CH % 2UL)
    {
        ...
        TMR4_OCO_SetLowChCompareMode(TMR4_UNIT, TMR4_OCO_CH, &stcLowChCmpMode);  /* Set OCO low channel compare mode */
    }
