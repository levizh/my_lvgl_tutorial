/**
 *******************************************************************************
 * @file  usart/uart_hwflowctrl_rts/source/main.c
...
/**
 * @addtogroup UART_hwflowctrl_rts
 * @{
 */
...
#define USART_RTS_PORT                  (GPIO_PORT_E)   /* PE5: USART6_RTS */
#define USART_RTS_PIN                   (GPIO_PIN_05)
#define USART_RTS_GPIO_FUNC             (GPIO_FUNC_7_USART6_RTS)
...
int32_t main(void)
{
    ...
    const stc_usart_uart_init_t stcUartInit = {
        ...
        .u32HwFlowCtrl = USART_HWFLOWCTRL_RTS,
    };

    stc_ring_buffer_t stcRingBuf = {
        .u16InIdx = 0,
        .u16OutIdx = 0,
        .u16UsedSize = 0,
        .u16Capacity = sizeof (stcRingBuf.au8Buf),
    };
    ...
    GPIO_SetFunc(USART_RTS_PORT, USART_RTS_PIN, USART_RTS_GPIO_FUNC, PIN_SUBFUNC_DISABLE);
    GPIO_Lock();
    ...
    do
    {
        ...
        u8Data = (uint8_t)USART_RecData(USART_UNIT);
        RingBufWrite(&stcRingBuf, u8Data);
    } while (u8Data != (uint8_t)'\n'); /* Wait the newline character '\n' */

    /* Send back test data */
    for (i = 0U; i < stcRingBuf.u16InIdx; i++)
    {
        while (Reset == USART_GetFlag(USART_UNIT, USART_FLAG_TXE)) /* Wait Tx data register empty */
        {
        }

        RingBufRead(&stcRingBuf, &u8Data);
